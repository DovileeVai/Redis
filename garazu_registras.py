import json
from collections import OrderedDict # cia tam, kad gaunama garazo informacija butu surasyta tokia tvarka, kaip parodyta, nes siaip ne tokia tvarka atspasdina
import re       # biblioteka regexui
import werkzeug # ziniatinklio serverio valdymui ir HTTP uzklausu apdorojimui (naudojama kartu su "Flask")
import redis    # saveika su "Redis" duomenu baze
from flask import (Flask, request, jsonify, abort) # komponentai is "Flask", naudojami kurti HTTP marsrutus, apdoroti uzklausas ir siusti atsakymus

licenseRegex = "^[A-Z0-9]{1,7}$"

# si funkcija sukuria "Flask" programa ir sujungia ja su "Redis" duomenu baze
def create_app(): 
    app = Flask(__name__)
    # sukuriamas klientas, kuris jungiasi prie "Redis" serverio per 6379 prievada
        ...
        if search(garageId):
            ...
            if int(spotNo) <= spotsCount and int(spotNo) > 0:
                ...
                if licenseNo: 
                    ...
                    return { "message": "Vieta laisva"}, 204 
            else:
